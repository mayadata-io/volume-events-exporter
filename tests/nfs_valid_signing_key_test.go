...
var _ = Describe("TEST NFS PVC CREATE & DELETE EVENTS WITH SIGNING KEY VALIDATIONS", func() {
	var (
		// PVC configuration
		accessModes    = []corev1.PersistentVolumeAccessMode{corev1.ReadWriteMany}
		capacity       = "1Gi"
		pvcName        = "signing-event-nfs-pvc"
		scName         = "openebs-rwx"
		nfsPVName      string
	...
	When("signing key path information is added on deployment", func() {
		It("should have signing key information", func() {
			...
			Expect(err).To(BeNil(), "while updating deployment with signatures")
		})
	})
